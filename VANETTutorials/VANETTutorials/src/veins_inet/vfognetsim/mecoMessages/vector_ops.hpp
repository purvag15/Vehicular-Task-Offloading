...
namespace machine_learning
{
    ...
    std::valarray<T> pop_front(const std::valarray<T> &A)
    {
        ...
        for (size_t i = 1; i < A.size();
             i++)
        {                    // // For every (except first) element in A
    ...
    std::valarray<T> pop_back(const std::valarray<T> &A)
    {
        ...
        for (size_t i = 0; i < A.size() - 1;
             i++)
        {                // For every (except last) element in A
            B[i] = A[i]; // Copy element in B
        }
        return B; // Return resultant vector
    }
    ...
    template <typename T>
    std::pair<std::vector<std::vector<std::valarray<T>>>, std::vector<std::vector<double>>> minmax_scaler_minmax(
        const std::vector<std::vector<std::valarray<T>>> &A, const T &low,
        const T &high)
    {
        // print dimensions of A
        // std::cout << "A dimensions: " << A.size() << " " << A[0].size() << std::endl;
        ...
        // As this function is used for scaling training data vector should be of
        // shape (1, X)
        // std::cout << shape.second << "shape" << std::endl;
        std::vector<std::vector<double>> minmax(shape.second, std::vector<double>(2, 0));
        if (shape.first != 1)
        {
            std::cerr << "ERROR (" << __func__ << ") : ";
            std::cerr
        ...
        return make_pair(B, minmax); // Return new resultant 3D vector
    }
